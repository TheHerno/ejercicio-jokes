...
import (
	"net/http"
	"strings"
	clientRouter "test-project-hernan/src/environments/client/resources/router"
	"test-project-hernan/src/libs/env"
	myErrors "test-project-hernan/src/libs/errors"
	"test-project-hernan/src/libs/middleware"
	"test-project-hernan/src/libs/sentry"
	"test-project-hernan/src/utils"
	"time"
...
func pingEndpoint(muxRouter *mux.Router) {
	muxRouter.HandleFunc("/ping", func(response http.ResponseWriter, request *http.Request) {
		bodyResponse := struct {
			...
			Service:  "test-project-hernan is online",
			Datetime: time.Now(),
		}
